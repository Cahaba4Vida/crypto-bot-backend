...
exports.handler = async (event) => {
  try {
    ...
    if (!text) {
      return buildResponse(400, { error: 'text is required.' });
    }

    const prompt = [
      'Extract positions from the following text export.',
    ...
    console.error('Unexpected error parsing text.', error);
    return buildResponse(500, { error: 'Unexpected error parsing text.' });
  }
};
